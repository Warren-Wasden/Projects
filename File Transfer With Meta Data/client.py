...
class FileTransferClient:
    ...
    def send_file(self, filepath):
        ...
        try:
            print("\n\n")
            #read file, encode and pack metadata, compute the hash, concatenate everything, and send it to server
            file_data = self.read_file(filepath)
            meta_data = self.encode_metadata(filepath)
            hash_value = self.compute_hash(meta_data + file_data)
            message = meta_data + file_data + hash_value

            self.client_socket.send(message)

            server_hash = self.client_socket.recv(128)

            if server_hash == hash_value:
                print("File sent successfully. ")
                return True
            else:
                print("Hash value from server does not equal hash value. ")
                return False
    ...
    def encode_metadata(self, filepath):
        ...
        created_date_length = len(created_date)
        
        encoded_metadata = pack(f'!HBB{filename_length}s{file_type_length}s{created_date_length}sIB'
                                , filename_length, file_type_length, created_date_length,
                                filename.encode(), file_type.encode(), created_date.encode(), 
                                file_size, self.hash_length
                                )
    ...
    def compute_hash(self, data):
        ...
        return hash_object.digest(self.hash_length)
...
if __name__ == "__main__":
    client = FileTransferClient("localhost", 5000)
    #client = FileTransferClient("localhost", 12356)
    if client.start():
        client.send_file("source/test1.jpg")
