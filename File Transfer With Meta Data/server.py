...
class FileTransferServer:
                ...
                try:
                    ...
                    with connected_Socket:
                        while self.is_running:
                            ...
                            #recieve metadata
                            metadata = connected_Socket.recv(1024)
                            if not metadata:
                                print("Client disconnected...\n")
                                break

                            #Unpack metadata
                            filename, file_extension, created_date, file_size, hash_length, remaining_file_data = self.unpack_metadata(metadata)
                            file_data = remaining_file_data

                            #finish recieving rest of file
                            while len(file_data) < file_size:
                                remaining_bytes = min(1024, file_size - len(file_data))
                                next_data = connected_Socket.recv(remaining_bytes)
                            ...
                            #recieve hash after file data collected
                            client_hash = connected_Socket.recv(hash_length)
                            
                            #write file, compute hash and send to server
                            all_data = metadata[:9+len(filename) + len(file_extension) + len(created_date)] + file_data 
                            
                            self.write_file(filename, file_data)
                            server_hash = self.compute_hash(all_data, hash_length)
                            connected_Socket.send(server_hash)

                            
                            if server_hash == client_hash:
                                print("File received successfully.")
                            else:
                                print("Hash verification failed for file")
    ...
    def unpack_metadata(self, data):
        ...
        """
        # TODO: Implement the functionality described in this function's docstring
        filename_length, file_extension_length, created_length = unpack('!HBB', data[:4])
        increment = 4
    
        filename = data[increment : increment + filename_length].decode()
        increment += filename_length

        file_extension = data[increment : increment + file_extension_length].decode()
        increment += file_extension_length

        created = data[increment : increment + created_length].decode()
        increment += created_length
    
        file_size, hash_length = unpack('!IB', data[increment : increment + 5])
        increment += 5

        remaining_data = data[increment:]

        return filename, file_extension, created, file_size, hash_length, remaining_data
...
if __name__ == "__main__":
    server = FileTransferServer(5000)
    #server = FileTransferServer(12356)
    if server.start():
        server.run()
